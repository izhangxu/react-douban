...
export const fetchData = (state = defaultState, action = {}) => {
	switch (action.type) {
		case GET_MOVIES_REQUEST:
			if (action.start) {
				return Immutable.Map({ data: state.get('data'), isFetching: true });
			}
			return Immutable.Map({ data: [], isFetching: true });
		case GET_MOVIES_SUCCESS:
			if (action.start) {
				return Immutable.Map({ data: state.get('data').concat(action.json), isFetching: false });
			}
